...
client.on("ready", () => {
    try {
        ...
        client.guilds.forEach(guild => {
            if (!guild.roles.get(config.userrole)) console.log(`${guild.name} has no userrole or the snowflake that was given in the config file is invalid. WARNING THE BOT HAS PROBALLY CRASHED!!!`);
        });
    } catch (e) {
            ...
            if (message.content === `${config.prefix}verify`) {
                ...
                if (config.captchaType == "image") {
                    ...
                    message.author.send(new Discord.RichEmbed()
                        ...
                        .setDescription(`Please type !verify and then type the numbers seen below into (${message.channel}) (click) in order to verify your account.`)
                        .setColor("RANDOM")
                        .setTimestamp()
                ...
                message.channel.awaitMessages(msg => msg.content === config.prefix + "verify " + captchaInstance.captcha && msg.author === message.author, {
                    ...
                    .then(() => {
                        ...
                        if (logChannel && logChannel.type === "text") logChannel.send(`You was successfully verified.`);
